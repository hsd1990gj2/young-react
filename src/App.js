...
let reducer = function reducer(state = {
    n: 0,
    m: 0
}, action) {
    //reducer就是根据不同的行为标识来修改状态信息的
    switch (action.type) {
        case 'SUPPORT_A': state.n = state.n+1;break;
        case 'SUPPORT_B': state.m = state.m+1;break;
    }
    return state;   //return的是什么，就会把store中的状态改成什么
}
let store = createStore(reducer);
//把创建的容器挂载到全局下，保证每一个子组件都可以获取到store,从而执行一些其它的操作（当然也可以基于属性）
window.store = store;
